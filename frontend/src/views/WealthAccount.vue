...
const wealthAccounts = computed(() => accounts.value.filter((a) => a.type === 'wealth'))
...
const tradedSymbolMap = computed(() => {
  ...
  for (const t of accountTxns.value) {
    ...
    const nm = (t.remark || '')
      .replace(/^(理财申购|理财赎回|银行理财产品申购|银行理财产品赎回|申购|赎回)[：:]/, '')
      .replace(/\s*\d.*$/, '')
      .trim()
...
const realizedPnl = computed(() =>
  accountTxns.value
    ...
    .map((t) => {
      ...
      return {
        id: t.id, occurred_at: t.occurred_at, name, activity: '银行理财产品赎回',
...
const summary = computed(() => {
  ...
  return { inflow, outflow, market, realized: realizedTotal.value, avail, total: avail + market }
})
...
function txnActivity(t: Transaction): string {
  ...
  if (t.trade_symbol) return t.type === 'income' ? '理财赎回' : '理财申购'
...
async function load() {
  ...
  if (qid && wealthAccounts.value.some((a) => a.id === qid)) {
    selectedAccountId.value = qid
  } else if (!selectedAccountId.value && wealthAccounts.value.length) {
    selectedAccountId.value = wealthAccounts.value[0].id
  }
  await loadTxns()
}
...
function onEditTxn(row: Transaction) {
  ...
  const nameFromRemark = (row.remark || '').replace(/^(理财申购|理财赎回|申购|赎回)[：:]/, '').replace(/\s*\d.*$/, '')
  ...
  <div class="wealth-page">
    ...
    <div class="panel">
      <div class="panel-head">
        <span class="panel-title">{{ selectedAccount?.name || '银行理财账户' }}</span>
        <span class="panel-balance" :class="{ neg: summary.total < 0 }">{{ fmt(summary.total) }}</span>
        <div class="head-spacer" />
        <el-select v-if="wealthAccounts.length > 1" v-model="selectedAccountId" size="small" style="width:160px">
          <el-option v-for="a in wealthAccounts" :key="a.id" :label="a.name" :value="a.id" />
        </el-select>
        <el-select v-model="holdingScope" size="small" style="width:180px">
          <el-option label="当前持仓理财产品" value="current" />
          <el-option label="所有交易过的产品" value="all" />
        </el-select>
      </div>

      <!-- 银行理财：产品名称/机构/累计金额/占比%/购买日/到期日/预计年收益率% -->
      <el-table :data="displayHoldings" size="small" border>
        <el-table-column label="产品名称" min-width="200" fixed="left">
          <template #default="{ row }">{{ row.symbol ? row.symbol + ' ' : '' }}{{ row.name }}</template>
        </el-table-column>
        <el-table-column label="机构" min-width="130">
          <template #default="{ row }">{{ wealthInfo(row)?.issuer || '—' }}</template>
        </el-table-column>
        <el-table-column label="累计金额" align="right" min-width="130">
        ...
        <el-table-column label="购买日" min-width="120">
          <template #default="{ row }">{{ wealthInfo(row)?.start_date || '—' }}</template>
        </el-table-column>
        <el-table-column label="到期日" min-width="120">
          <template #default="{ row }">{{ wealthInfo(row)?.end_date || '—' }}</template>
        </el-table-column>
        <el-table-column label="预计年收益率%" align="right" min-width="130">
          <template #default="{ row }">{{ wealthInfo(row)?.expected_rate != null ? Number(wealthInfo(row)!.expected_rate).toFixed(2) : '—' }}</template>
        </el-table-column>
    ...
    <div class="panel">
      <div class="panel-head">
        <div class="head-spacer" />
        <el-button size="small" type="primary" @click="onRecord('buy')">理财申购</el-button>
        <el-button size="small" @click="onRecord('sell')">理财赎回</el-button>
    ...
    <!-- 转账弹窗 -->
    <el-dialog v-model="transferDialog" title="理财账户转账" width="90%" style="max-width:460px" :close-on-click-modal="false">
      <el-form label-width="90px">
        <el-form-item label="理财账户">
...
<style scoped>
.wealth-page { padding: 12px 16px; display: flex; flex-direction: column; gap: 14px; }
...
.head-spacer { flex: 1; }
.wealth-summary { display: flex; flex-wrap: wrap; gap: 28px; margin-top: 12px; padding: 10px 16px; background: #f7f8fa; border-radius: 6px; }
