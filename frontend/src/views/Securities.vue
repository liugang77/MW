...
// 持仓产品范围：current=当前持仓产品；all=所有交易过的产品（含已清仓）
const holdingScope = ref<'current' | 'all'>('current')
// 底部标签页：交易明细 / 历史盈亏
const bottomTab = ref<'txns' | 'pnl'>('txns')

// 证券类账户：股票 / 债券 / 逆回购 / 期货 / 融资融券
const SECURITY_TYPES = ['stock', 'bond', 'reverse_repo', 'futures', 'margin']
const investAccounts = computed(() => accounts.value.filter((a) => SECURITY_TYPES.includes(a.type)))

const selectedAccount = computed(() => accounts.value.find((a) => a.id === selectedAccountId.value) || null)
// 记账按钮文案：证券买入 / 卖出
const buyLabel = '证券买入'
const sellLabel = '证券卖出'
...
// 当前账户内曾经交易过的标的（来自带 trade_symbol 的流水）：symbol -> name
const tradedSymbolMap = computed(() => {
  const m = new Map<string, string>()
  for (const t of accountTxns.value) {
    if (!t.trade_symbol) continue
    // 仅统计实际持仓过的标的：跳过“申购中/未中签”的新股流水
    if (t.ipo_status === 'pending' || t.ipo_status === 'lost') continue
    const nm = (t.remark || '')
      .replace(/^(买入|卖出|新股中签)[：:]/, '')
      .replace(/\s*\d.*$/, '')
      .trim()
...
// 展示用持仓：当前持仓 + （范围为「所有交易过的产品」时）已清仓的标的补 0 行
  ...
  if (holdingScope.value === 'all') {
    ...
    for (const [sym, name] of tradedSymbolMap.value) {
      ...
      rows.push({
        ...
        symbol: sym, name, type: selectedAccount.value?.type || 'stock',
...
// 历史盈亏：卖出/赎回流水（income + trade_symbol）的已实现盈亏
const realizedPnl = computed(() =>
  accountTxns.value
    ...
    .map((t) => {
      ...
      return {
        id: t.id, occurred_at: t.occurred_at, name,
        activity: '证券卖出',
...
function txnActivity(t: Transaction): string {
  ...
  if (t.ipo_status === 'pending') return '新股申购'
  if (t.ipo_status === 'won') return '新股中签'
  if (t.ipo_status === 'lost') return '新股未中签'
  if (t.ipo_status === 'refund') return '申购返款'
  if (t.trade_symbol) return t.type === 'income' ? '证券卖出' : '证券买入'
...
async function load() {
  ...
  if (!selectedAccountId.value && investAccounts.value.length) {
    selectedAccountId.value = investAccounts.value[0].id
  }
  await loadTxns()
}
...
// 新股申购：本质为证券买入（建仓），仅标题不同
function onIpo() {
  tradeStore.open('buy', selectedAccountId.value ?? undefined, { ipo: true })
}

// 中签确认：对之前的新股申购做中签/未中签确认
function onIpoConfirm() {
  ipoStore.open(selectedAccountId.value ?? undefined)
}

// 资金转账：在资金账户与投资账户之间划转（转入/转出）
...
function canEditTxn(t: Transaction): boolean {
  // 新股申购/中签类流水不走买卖编辑（请在列表里删除后重新操作）
  if (t.ipo_status) return false
  ...
  <div class="sec-page">
    ...
    <div class="panel">
      <div class="panel-head">
        <span class="panel-title">{{ selectedAccount?.name || '证券账户' }}</span>
        <span class="panel-balance" :class="{ neg: summary.total < 0 }">{{ fmt(summary.total) }}</span>
        <div class="head-spacer" />
        <el-button size="small" :loading="syncingQuote" @click="syncQuote">同步行情</el-button>
        <el-select v-model="holdingScope" size="small" style="width:160px">
          <el-option label="当前持仓产品" value="current" />
          <el-option label="所有交易过的产品" value="all" />
        </el-select>
      </div>

      <!-- 持仓列表（证券：证券名称/持仓数量/持仓成本/市值/占比%/浮动盈亏/均价/现价/浮动收益率%） -->
      <el-table :data="displayHoldings" size="small" border>
        <el-table-column label="证券名称" min-width="200" fixed="left">
          <template #default="{ row }">{{ row.symbol ? row.symbol + ' ' : '' }}{{ row.name }}</template>
        </el-table-column>
        <el-table-column label="持仓数量" align="right" min-width="110">
        ...
        <el-table-column label="均价" align="right" min-width="100">
    ...
    <div class="panel">
      <div class="panel-head">
        <div class="head-spacer" />
        <el-button size="small" type="primary" @click="onRecord('buy')">{{ buyLabel }}</el-button>
        <el-button size="small" @click="onRecord('sell')">{{ sellLabel }}</el-button>
        <el-button size="small" @click="onIpo">新股申购</el-button>
        <el-button size="small" @click="onIpoConfirm">中签确认</el-button>
      ...
      <el-table :data="accountTxns" size="small" border>
        ...
        <el-table-column label="佣金" align="right" min-width="90">
          <template #default="{ row }">{{ tradeMeta(row).commission }}</template>
        </el-table-column>
        <el-table-column label="总费用" align="right" min-width="90">
...
<style scoped>
.sec-page { padding: 12px 16px; display: flex; flex-direction: column; gap: 14px; }
...
.head-spacer { flex: 1; }
.sec-summary { display: flex; flex-wrap: wrap; gap: 28px; margin-top: 12px; padding: 10px 16px; background: #f7f8fa; border-radius: 6px; }
