...
const METAL_TYPES = ['metal', 'metal_td']
const metalAccounts = computed(() => accounts.value.filter((a) => METAL_TYPES.includes(a.type)))
...
// 展示用持仓：当前持仓 +（范围为「所有交易过的品种」时）已清仓的标的补 0 行
  ...
  if (holdingScope.value === 'all') {
    ...
    for (const [sym, name] of tradedSymbolMap.value) {
      ...
      rows.push({
        ...
        symbol: sym, name, type: selectedAccount.value?.type || 'metal',
...
// 历史盈亏：卖出流水（income + trade_symbol）的已实现盈亏
const realizedPnl = computed(() =>
  accountTxns.value
    ...
    .map((t) => {
      ...
      return {
        id: t.id, occurred_at: t.occurred_at, name, activity: '贵金属卖出',
...
function txnActivity(t: Transaction): string {
  ...
  if (t.trade_symbol) return t.type === 'income' ? '贵金属卖出' : '贵金属买入'
...
async function load() {
  ...
  if (qid && metalAccounts.value.some((a) => a.id === qid)) {
    selectedAccountId.value = qid
  } else if (!selectedAccountId.value && metalAccounts.value.length) {
    selectedAccountId.value = metalAccounts.value[0].id
  }
  await loadTxns()
}
...
async function syncQuote() {
  ...
  try {
    ...
    if (r.updated) ElMessage.success(`已获取 ${r.updated} 个品种的最新价格`)
    else ElMessage.info('暂无可获取价格的持仓品种')
    if (r.failed?.length) ElMessage.warning(`未成功：${r.failed.join('、')}`)
    await load()
  } catch (e) {
    ElMessage.error('获取价格失败，请稍后重试')
  } finally {
...
function onEditTxn(row: Transaction) {
  ...
  // 证券账户=本投资账户（流水 to_account_id 关联，缺省取当前账户）；资金账户=流水 account_id
  ...
  const nameFromRemark = (row.remark || '').replace(/^(买入|卖出)[：:]/, '').replace(/\s*\d.*$/, '')
  ...
  <div class="metal-page">
    ...
    <div class="panel">
      <div class="panel-head">
        <span class="panel-title">{{ selectedAccount?.name || '贵金属账户' }}</span>
        <span class="panel-balance" :class="{ neg: summary.total < 0 }">{{ fmt(summary.total) }}</span>
        <div class="head-spacer" />
        <el-select v-if="metalAccounts.length > 1" v-model="selectedAccountId" size="small" style="width:160px">
          <el-option v-for="a in metalAccounts" :key="a.id" :label="a.name" :value="a.id" />
        </el-select>
        <el-button size="small" :loading="syncingQuote" @click="syncQuote">获取价格</el-button>
        <el-select v-model="holdingScope" size="small" style="width:170px">
          <el-option label="当前持仓贵金属" value="current" />
          <el-option label="所有交易过的品种" value="all" />
        </el-select>
      </div>

      <!-- 持仓：产品/持仓数量/买入均价/持仓成本/市值/占比%/浮动盈亏/浮动收益率%/最新行情 -->
      <el-table :data="displayHoldings" size="small" border>
        <el-table-column label="产品" min-width="200" fixed="left">
        ...
        <el-table-column label="买入均价" align="right" min-width="110">
          <template #default="{ row }">{{ fmt4(toNum(row.quantity) ? toNum(row.cost) / toNum(row.quantity) : 0) }}</template>
        </el-table-column>
        <el-table-column label="持仓成本" align="right" min-width="130">
        ...
        <el-table-column label="最新行情" align="right" min-width="110">
    ...
    <div class="panel">
      <div class="panel-head">
        <div class="head-spacer" />
        <el-button size="small" type="primary" @click="onRecord('buy')">贵金属买入</el-button>
        <el-button size="small" @click="onRecord('sell')">贵金属卖出</el-button>
    ...
    <!-- 转账弹窗 -->
    <el-dialog v-model="transferDialog" title="贵金属账户转账" width="90%" style="max-width:460px" :close-on-click-modal="false">
      <el-form label-width="90px">
        <el-form-item label="贵金属账户">
...
<style scoped>
.metal-page { padding: 12px 16px; display: flex; flex-direction: column; gap: 14px; }
...
.head-spacer { flex: 1; }
.metal-summary { display: flex; flex-wrap: wrap; gap: 28px; margin-top: 12px; padding: 10px 16px; background: #f7f8fa; border-radius: 6px; }
